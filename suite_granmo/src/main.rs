//! # Suite of Granmo Models
//! - TFIDF
//! - Byte-Bag
//! - Byte-Conv (convolutional TM)
//! - Clustering
//! - Ensemble Models (e.g. SeqFreq: Byte-Conv (Sequence) + Byte-Bag (Frequency))
//!
//! Model Development Tools;
//! - Batch Comparison Reports
//! - Pre-processing Specification & Comparison
//!
//!
//! Other Experiments:
//! - Team Diversity Layer
//! - Feature-Derivation Layer
//! - Fire Guard (Dead 'Weight' Pruning)
//!
//!
...
pub fn run_single_experiment(
    ...
    //             .map(|(d, l)| (d, *l))
    //             .ok_or(GranmoModelError::BctIndexOutOfRange)?;
    //         engine.engine_train_step(document, label, &mut rng)?;
    //     }
    // }
...
fn print_experiment_report(run_label: &str, report: &ExperimentReport) {
    ...
    println!("  Training Time Sec:     {:.2}s", report.train_seconds);
    println!("------------------------------------------------------------");
    println!(
        "  Accuracy (@ V > 0): {:.2}%",
...
fn handle_train(args: &CliArgs) -> Result<(), GranmoModelError> {
    ...
    // applies to the byte-bag engine and the hybrid's bag bank only
    if run_config.fire_guard_streak_limit != FireGuardStreakLimit::DISABLED
        && run_config.engine_selection == EngineSelection::ByteConv
    {
        println!(
            "note: --fire-guard applies to the byte-bag engine and the seq-freq-hybrid's \
                     bag bank only; it is ignored for byte-conv this run (conv port is recorded backlog)"
        );
    }
