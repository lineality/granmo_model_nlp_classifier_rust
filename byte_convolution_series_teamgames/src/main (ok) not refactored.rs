...
impl ClassifierEngine {
    /// Stable engine identifier for reports, run labels, and (Drop 2.2c-ii)
    /// the `--engine` CLI value.
    ...
    pub fn vote_sum(&self, document: &[u8]) -> Result<i32, GranmoModelError> {
        match self {
            Self::ByteConv(conv_engine) => conv_engine.vote_sum(document),
    ...
    pub fn predict(
        ...
        match self {
            Self::ByteConv(conv_engine) => conv_engine.predict(document, decision_threshold),
    ...
    pub fn train_step(
        ...
        match self {
            Self::ByteConv(conv_engine) => conv_engine.train_step(document, label_is_positive, rng),
    ...
    /// Fired-clause bitset (embedding deliverable §7.2; also the input the
    /// fire-rate report of Drop 2.2c-ii folds over — that report must be
    /// engine-agnostic, which is why this lives on the enum).
    pub fn fired_clause_bits(&self, document: &[u8]) -> Result<Vec<u64>, GranmoModelError> {
        match self {
            Self::ByteConv(conv_engine) => conv_engine.fired_clause_bits(document),
    ...
    pub fn describe_clause(
        ...
        match self {
            Self::ByteConv(conv_engine) => {
                conv_engine.describe_clause(clause, max_rendered_literals)
            }
            Self::ByteBag(bag_engine) => {
    ...
    pub fn validate_internal_consistency(&self) -> Result<(), GranmoModelError> {
        match self {
            Self::ByteConv(conv_engine) => conv_engine.validate_internal_consistency(),
    ...
    pub fn clause_count(&self) -> usize {
        match self {
            Self::ByteConv(conv_engine) => conv_engine.clause_count(),
    ...
    pub fn build_probability_lut(&self) -> Result<ProbabilityLut, GranmoModelError> {
        match self {
            Self::ByteConv(conv_engine) => conv_engine.build_probability_lut(),
...
impl ByteBagTM {
    ...
    pub fn new_with_vocabulary(
        ...
        vocabulary_for_engine.validity_recheck()?;
    ...
    pub fn bag_validate_internal_consistency(&self) -> Result<(), GranmoModelError> {
        ...
        self.bag_vocabulary.validity_recheck()?;
...
mod tests {
    ...
    */
    ...
    fn bag_classifier_engine_dispatch_identity() {
        ...
        lut_for_bag.validity_recheck().unwrap();
    }
